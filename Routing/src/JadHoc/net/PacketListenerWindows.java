...
*
*/
public class PacketListenerWindows extends Thread {
	...
	*/
	public PacketListenerWindows(ConfigInfo cfg, CurrentInfo cur, RouteManager rm) {
		cfgInfo = cfg;
		curInfo = cur;
		rtMgr = rm;
		...
		while(true) {
			...
			try  {
				...
				if(pktInfo.packet instanceof ARPPacket) {
					...
					if(getInetByAddress(ipAddr).equals(cfgInfo.ipAddressGatewayVal)) {
			...
			if(((EthernetPacket)
				...
				synchronized(pktQueue) {
					pktQueue.add(pktQueue.size(),
							new PacketInfo(PacketInfo.PACKET_REQUIRES_ROUTE,
								pkt));
				}
			} else {
				synchronized(pktQueue) {
