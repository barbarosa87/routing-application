...
* Handler. It provides methods to retrive, update & validate
...
public class ConfigInfo {
	...
	*/
	public ConfigInfo(String cfg) throws Exception {
		cfgFileName = cfg;
	...
	/**
	* Constructor to create a default value filed ConfigInfo
	* information object.
	*/
	public ConfigInfo() {
	}
			   ...
			   || strLine.charAt(0) == '#') {
				...
				if(strTag.equals(executionModeStr)) {
					...
					executionModeVal = strValue;
					    ...
					    && !strValue.toLowerCase().equals(ZAURUS_OS)) {
	...
	*/
		...
		if(!tempCfg.osInUse.toLowerCase().equals(WINDOWS_OS)
		    ...
		    && !tempCfg.osInUse.toLowerCase().equals(ZAURUS_OS)) {
			throw new Exception("Invalid osInUse - ("
						+ WINDOWS_OS + " or "
						+ LINUX_OS + " or "
						+ ZAURUS_OS + ")");
